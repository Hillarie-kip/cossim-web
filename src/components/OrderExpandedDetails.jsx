...
export default function OrderExpandedDetails({ order }) {
  ...
  const [payments, setPayments] = useState(order.ShipmentOrderPayment || []);
  const [history, setHistory] = useState([]);
  const [loading, setLoading] = useState(true);
  const orderNO = order.OrderNO;

  const loadPayments = async () => setPayments(list(await getShipmentOrderPayment({ orderNO })));
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (paymentResult.status === "fulfilled") setPayments(list(paymentResult.value));
  ...
  return <div className="row g-3 p-2 bg-light">
